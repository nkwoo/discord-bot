...
export class LeagueOfLegendImpl implements LeagueOfLegend {
        ...
        channel.send("데이터 조회중......").then((editMsg) => {
            this.htmlParser.requestHeaderData<ApiSummonerInfo>(HttpMethod.GET, `https://kr.api.riotgames.com/lol/summoner/v4/summoners/by-name/${encodeURI(nickname)}`, this.headerJson).then((json) => {
                if (json === undefined || json.status != 200) {
                    editMsg.edit("데이터 조회 실패 ❌").then(() => channel.send("유저정보를 가져올 수 없습니다.."));
                    return;
                }
                ...
                if (profileIcon === undefined || resourceVersion === undefined || champion === undefined) {
                    editMsg.edit("데이터 조회 실패 ❌").then(() => channel.send("필수 정보를 불러올 수 없습니다.."));
                    return;
                }
                ...
                const loadingData = new Promise((resolve, reject) => {
                    ...
                    this.htmlParser.requestHeaderData<ApiLeagueInfo[]>(HttpMethod.GET, `https://kr.api.riotgames.com/lol/league/v4/entries/by-summoner/${summonerInfo.id}`, this.headerJson).then((json) => {
                        if (json === undefined || json.status != 200 || json.data === undefined) {
                            editMsg.edit("데이터 조회 실패 ❌").then(() => channel.send("유저 정보를 찾을 수 없습니다.."));
                            reject("유저 정보를 찾을 수 없습니다.");
                            return;
                        }
                    ...
                    this.htmlParser.requestHeaderData<ApiMatchInfo>(HttpMethod.GET, `https://kr.api.riotgames.com/lol/match/v4/matchlists/by-account/${summonerInfo.accountId}?endIndex=8`, this.headerJson).then((json) => {
                        if (json === undefined || json.status != 200) {
                            editMsg.edit("데이터 조회 실패 ❌").then(() => channel.send("오류가 발생하였습니다.."));
                            reject("오류가 발생하였습니다.");
                            return;
                        }

                        json.data.matches.forEach(value => {
                ...
                loadingData.then(() => {
                    editMsg.edit("데이터 조회 성공 ✅").then(() => {
                        channel.send(new RichEmbed()
                            ...
                            .setURL(`https://www.op.gg/summoner/userName=${encodeURI(nickname)}`)
    ...
    getRotationsChampion(channel: TextChannel | DMChannel | GroupDMChannel): void {
        channel.send("데이터 조회중......").then((editMsg) => {
            this.htmlParser.requestHeaderData<ApiRotationsChampionInfo>(HttpMethod.GET, LOL_ROTATION_CHAMPION_URL, this.headerJson).then((json) => {
                if (json === undefined) {
                    editMsg.edit("데이터 조회 실패 ❌").then(() => channel.send("로테이션 정보를 가져올 수 없습니다.."));
                    return;
                }
                ...
                if (champion === undefined) {
                    editMsg.edit("데이터 조회 실패 ❌").then(() => channel.send("챔피언 정보를 가져올 수 없습니다.."));
                    return;
                }
